...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/kuhingg/ip/main/vps | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/kuhingg/ip/main/vps | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/kuhingg/ip/main/vps | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/kuhingg/ip/main/vps | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/kuhingg/ip/main/vps | grep $ipsaya | awk '{print $3}')
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    clear
    curl -s ipinfo.io/city >>/etc/city/city
    curl -s ipinfo.io/org | cut -d " " -f 2-10 >>/etc/city/isp
...
wget raw.githubusercontent.com/sanakstore1/sanakstore/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
function menu(){
    ...
    wget --no-check-certificate https://raw.githubusercontent.com/sanakstore1/VVIP/main/menu/menu.zip
    unzip menu.zip
    chmod +x menu/*
